	...
	for(int kk=0;kk<3;kk++)
        {
		  std::chrono::steady_clock::time_point begin1
				  = std::chrono::steady_clock::now();
		  stan::math::matrix_gpu m_gpu(m1_cpu);
		  stan::math::cholesky_decompose(m_gpu, i, j, k);
		  stan::math::copy(m1_cl, m_gpu);   
		  //m1_cl = stan::math::cholesky_decompose(m1_cpu);
		  std::chrono::steady_clock::time_point end1
				  = std::chrono::steady_clock::now();
		  size_t current1
			  = std::chrono::duration_cast<std::chrono::nanoseconds>(end1 - begin1)
					.count();
          std::cout << size << " ------- " << i << "\t" << j << "\t" << k << "\t" << kk << "\t" << current1 << std::endl;
        }
      /*}*/
  /*  }
  }*/
...
TEST(MathMatrix, cholesky_decompose_small) {
  ...
  cholesky_decompose_test(200);
  cholesky_decompose_test(500);
  cholesky_decompose_test(750);
  cholesky_decompose_test(1000);
  cholesky_decompose_test(1500);
