...
TEST(MathMatrixCL, triangular_transpose_m_pass) {
  ...
  EXPECT_NO_THROW(
      m00.triangular_transpose<TriangularMapCL::LowerToUpper>());
  EXPECT_NO_THROW(m0_dst = stan::math::from_matrix_cl(m00.val()));
  ...
  EXPECT_NO_THROW(
      m11.triangular_transpose<TriangularMapCL::UpperToLower>());
  EXPECT_NO_THROW(m0_dst = stan::math::from_matrix_cl(m11.val()));
  EXPECT_EQ(1, m0_dst(0, 0));
