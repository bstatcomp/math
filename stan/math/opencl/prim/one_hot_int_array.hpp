#ifndef STAN_MATH_OPENCL_PRIM_ONE_HOT_INT_ARRAY_HPP
#define STAN_MATH_OPENCL_PRIM_ONE_HOT_INT_ARRAY_HPP
#ifdef STAN_OPENCL
...
inline auto one_hot_int_array(int K, int k) {
  static const char* function = "one_hot_int_array (OpenCL)";
  check_positive(function, "size", K);
  check_bounded(function, "k", k, 1, K);
  return select(row_index(K,1)==k-1, 1, 0);
}
}  // namespace math
}  // namespace stan
#endif
