 ...
 public:
  ...
  explicit matrix_cl(const Eigen::Matrix<T, R, C>& A)
    ...
    try {
      ...
      queue.enqueueWriteBuffer(buffer_cl_, opencl_context.blocking(),
       0, sizeof(T) * A.size(), A.data(), NULL, &transfer_event);
  ...
  explicit matrix_cl(const std::vector<T>& A, const int& R, const int& C)
    ...
    try {
      buffer_cl_ = cl::Buffer(ctx, CL_MEM_READ_WRITE, sizeof(T) * A.size());
      cl::Event transfer_event;
      queue.enqueueWriteBuffer(buffer_cl_, opencl_context.blocking(), 0, sizeof(T) * A.size(),
   ...
   */
  matrix_cl<T, enable_if_arithmetic<T>>& operator=(const matrix_cl<T, enable_if_arithmetic<T>>& a) {
  ...
  matrix_cl<T, enable_if_arithmetic<T>>& operator=(const matrix_cl<U, enable_if_arithmetic<T>>& a) {
    check_size_match("assignment of (OpenCL) matrices", "source.rows()",
                     a.rows(), "destination.rows()", rows());
    check_size_match("assignment of (OpenCL) matrices", "source.cols()",
                     a.cols(), "destination.cols()", cols());
