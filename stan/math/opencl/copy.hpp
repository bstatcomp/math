...
inline matrix_cl<T> to_matrix_cl(const Eigen::Matrix<T, R, C>& src) {
  ...
  try {
    ...
    queue.enqueueWriteBuffer(dst.buffer(), opencl_context.blocking(), 0, sizeof(T) * dst.size(),
...
inline Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic> from_matrix_cl(
  ...
  try {
    ...
    cl::Event copy_event;
    const cl::CommandQueue queue = opencl_context.queue();
    queue.enqueueReadBuffer(src.buffer(), opencl_context.blocking(), 0,
...
inline std::vector<T> packed_copy(const matrix_cl<T>& src) {
  ...
  try {
    ...
    cl::Event copy_event;
    queue.enqueueReadBuffer(packed.buffer(), opencl_context.blocking(), 0,
...
inline matrix_cl<T> packed_copy(const std::vector<T>& src, int rows) {
  ...
  try {
    ...
    const cl::CommandQueue queue = opencl_context.queue();
    queue.enqueueWriteBuffer(packed.buffer(), opencl_context.blocking(), 0,
...
inline T from_matrix_cl_error_code(const matrix_cl<T>& src) {
  ...
  try {
    cl::Event copy_event;
    const cl::CommandQueue queue = opencl_context.queue();
    queue.enqueueReadBuffer(src.buffer(), opencl_context.blocking(), 0, sizeof(T), &dst,
                            &src.write_events(), &copy_event);
    copy_event.wait();
    src.clear_write_events();
  } catch (const cl::Error& e) {
...
inline matrix_cl<T> to_matrix_cl(const T& src) {
  ...
  try {
    cl::Event copy_event;
    const cl::CommandQueue queue = opencl_context.queue();
    queue.enqueueWriteBuffer(dst.buffer(), opencl_context.blocking(), 0, sizeof(T), &src,
