...
TEST(MathGpu, make_kernel) {
  ...
  auto test_kern = stan::math::opencl_kernels::transpose(cl::NDRange(m00.rows(), m00.cols()), m00_dst);
  cl::Event test_event = test_kern(m00_dst.buffer(), m00.buffer(), m00.rows(), m00.cols());
  copy(m0_dst, m00_dst);
}
#endif
